...
def c_gen_dataframe(rfilename, num_event, num_start=0):
    """
...
def c_prepare_dataset(rfilename, num_event, dR, num_start=0):
    """
    ...
    df_pf_list, df_gen_list = c_gen_dataframe(rfilename, num_event, num_start)
...
def main_classification(dR = 0.4):
    start = timer()

    iname = r"D:\Data\Research\PUPPI\WJetsToQQ_HT-800toInf\output_1.root"
    num_events_train = 140
    oname = r"C:\Users\jackm\PycharmProjects\PileupMitigation\Regression_Task\classification_data\train" + str(num_events_train)
    dataset_train = c_prepare_dataset(iname, num_events_train, dR)
    # save outputs in pickle format
    with open(oname, "wb") as fp:
        pickle.dump(dataset_train, fp)

    num_events_valid = 30
    oname = r"C:\Users\jackm\PycharmProjects\PileupMitigation\Regression_Task\classification_data\valid" + str(num_events_valid)
    dataset_test = c_prepare_dataset(iname, num_events_valid, dR, num_events_train)
    with open(oname, "wb") as fp:
        pickle.dump(dataset_test, fp)

    num_events_test = 30
    oname = r"C:\Users\jackm\PycharmProjects\PileupMitigation\Regression_Task\classification_data\test" + str(num_events_test)
    dataset_valid = c_prepare_dataset(
...
def main_regression(dR = 0.4):
    ...
    iname = r"D:\Data\Research\PUPPI\WJetsToQQ_HT-800toInf\output_1.root"
    num_events_train = 140
    oname = r"D:\Data\Research\PUPPI\W_processed\train" + str(num_events_train) + "_reco"
    ...
    fp.close()
    num_events_valid = 30
    oname = r"D:\Data\Research\PUPPI\W_processed\valid" + str(num_events_valid) + "_reco"
    dataset_test = r_prepare_dataset(iname, num_events_valid, dR, num_events_train)
    with open(oname, "wb") as fp:
        pickle.dump(dataset_test, fp)
    fp.close()
    num_events_test = 30
    oname = r"D:\Data\Research\PUPPI\W_processed\test" + str(num_events_test) + "_reco"
