...
def arg_parse():
    ...
    parser.set_defaults(model_type='Gated',
                        ...
                        pulevel=80,
                        lamb = 1e-4,
                        training_path=r"/depot/cms/users/jprodger/PUPPI/Physics_Optimization/Experiment120824/data/dataset_graph_puppi_10000",
                        validation_path=r"/depot/cms/users/jprodger/PUPPI/Physics_Optimization/Experiment120824/data/dataset_graph_puppi_val_5000",
                        save_dir=r"/depot/cms/users/jprodger/PUPPI/Physics_Optimization/Experiment120824/tuning_output/",
                        jet_type = "W",
                        num_select_LV = 2,
                        num_select_PU = 30,
                        act_fn = 'relu'
...
def train(dataset, dataset_validation, trial, args):
    ...
    args.num_select_LV = trial.suggest_int("num_select_LV", 2, 10)
    args.num_select_PU = trial.suggest_int("num_select_PU", 10, 70)
    
    args.hidden_dim = trial.suggest_categorical("hidden_dim", [64, 128, 256])
    
    args.lr = trial.suggest_float("lr", 1e-4, 1e-2, log=True)
    
    args.num_enc_layers = trial.suggest_int("num_enc_layers", 1, 5)
    args.num_dec_layers = trial.suggest_int("num_dec_layers", 1, 6)
    
    args.lamb = trial.suggest_float("lamb", 1e-6, 1e-2, log=True)
    args.act_fn = trial.suggest_categorical("act_fn", ["relu", "leakyrelu", "gelu"])
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(opt, 15, gamma=0.99)
    ...
    while converge == False:
        ...
        for batch in training_loader:
            ...
            if count_event % 500 == 0:
                ...
                wandb.log({"SSL/PUPPI_sigma_valid": valid_SSLMassSigma/valid_PUPPIMassSigma})
                ...
                if validation_metric < best_validation_metric:
                    ...
                    print("model is saved in " + path + "/best_valid_model"+str(trial.number)+".pt")
                    print(f"New validation metric: {best_validation_metric:.4f}")
                    torch.save(model.state_dict(), path +
                               "/best_valid_model"+str(trial.number)+".pt")
                ...
                if validation_metric >= best_validation_metric:
                    ...
                    if last_steady_event == count_event - 500:
...
def main():
    ...
    modelname = args_end.save_dir + 'best_valid_model'+s2+'.pt'
    if args_end.jet_type == "W":
        filelists = ["/depot/cms/users/jprodger/PUPPI/Physics_Optimization/Experiment120824/data/dataset_graph_puppi_test_20000"]
