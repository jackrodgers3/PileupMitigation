...
def get_args():
    ...
    parser.add_argument("--n_encoder", help="num layers in encoder")
    parser.add_argument("--n_decoder", help="num layers in decoder")
    parser.set_defaults(
        data_path = r"C:\Users\jackm\PycharmProjects\PileupMitigation\Supervised\data2/",
        save_dir = r"C:\Users\jackm\PycharmProjects\PileupMitigation\Supervised\save_dir2/",
        bs = 32,
        lr=3e-4,
        hd = 256,
        dr = 0.1,
...
def train(trial):
    ...
    args.bs = trial.suggest_categorical("batch_size", [32, 64, 128, 256, 512])
    args.lr = trial.suggest_float("learning_rate", 1e-5, 1e-3, log=True)
    args.hd = trial.suggest_categorical("hidden_size", [32, 64, 128, 256])
    args.n_encoder = trial.suggest_categorical("n_encoder", [2, 3, 4, 5])
    args.n_decoder = trial.suggest_categorical("n_decoder", [2, 3, 4, 5])
    args.dr = trial.suggest_float("dropout", 0, 0.3)
    ...
    wandb.init(
        project='GP_Supervised_Test_Scratch3',
        config=wandb_config
    )
    ...
    for epoch in tqdm(range(args.ne)):
        ...
        for batch in tqdm(train_dataloader):
            opt.zero_grad()
            pred = model.forward(batch.x, batch.edge_index, batch.batch)
            label = batch.y
    ...
    mu, sig = getStat(mass_rel_diff_SSL)
    vm = sig / (1 - mu)
