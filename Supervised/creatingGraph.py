...
def r_prepare_dataset(rfilename, num_event, dR, num_start=0):
    ...
    for num in range(len(df_pf_list)):
        ...
        for ijet in range(0, njets_matched_list[num]):
            ...
            # node features
            #node_features = df_pfcands.drop(df_pfcands.loc[:, ['PF_charge']], axis=1).drop(df_pfcands.loc[:, ['PF_fromPV']], axis=1).to_numpy()

            node_features = df_pfcands.to_numpy()
            ...
            pf_jm = torch.tensor([pf_jet[0].pt]).view(-1)
            ...
            # charge
            index_charge = 4
            charges = node_features[:, index_charge] + 1
            charges = charges.type(torch.long)
            ch_one_hot = torch.nn.functional.one_hot(charges)
            ch_one_hot = ch_one_hot.type(torch.float32)

            index_pv = 8
            pvs = node_features[:, index_pv]
            pvs = pvs.type(torch.long)
            pv_one_hot = torch.nn.functional.one_hot(pvs)
            pv_one_hot = pv_one_hot.type(torch.float32)

            node_features = torch.cat(
                (node_features[:, 0:3], pdgId_one_hot, ch_one_hot, node_features[:, 5:8], pv_one_hot), 1)

            if num == 0:
                print("NODE FEATURES: ", node_features)
                print("node_features dimension: ", node_features.shape)
                print("pdgId dimensions: ", pdgId_one_hot.shape)
                print("ch_one_hot dimensions: ", ch_one_hot.shape)
                print("pv_one_hot dimensions: ", pv_one_hot.shape)
            # node_features = node_features.type(torch.float32)
...
def main_regression(dR = 0.4):
    ...
    iname = "/depot/cms/users/gpaspala/WJetsToQQ_HT-800toInf/output_1.root"
    num_events_train = 5000
    oname = "/depot/cms/users/jprodger/PUPPI/Physics_Optimization/supervised_jet_pt/data/train" + str(num_events_train)
    ...
    oname = "/depot/cms/users/jprodger/PUPPI/Physics_Optimization/supervised_jet_pt/data/valid" + str(num_events_valid)
    dataset_test = r_prepare_dataset(iname, num_events_valid, dR, num_events_train)
    with open(oname, "wb") as fp:
        pickle.dump(dataset_test, fp)
    fp.close()
    num_events_test = 5000
    oname = "/depot/cms/users/jprodger/PUPPI/Physics_Optimization/supervised_jet_pt/data/test" + str(num_events_test)
