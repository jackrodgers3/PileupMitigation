...
def generate_mask(dataset):
    ...
    #TODO: move this to global variable and input hyperparameter values from text file to cluster after bayesopt is done running
    bparams = []

    #TODO: comment out after bayesopt is done running
    f = open("datphys6dropout.txt", "r")
    s = f.read()
    while s != "":
        snum = s.find(" ")
        bparams.append(s[0:snum])
        s = s[snum+1:]
    ...
    for graph in dataset:
        ...
        num_select_LV = int(bparams[2])
        num_select_PU = int(bparams[3])
...
def clusterJets(pt, eta, phi, ptcut=0., deltaR=0.8):
    """
...
def main(modelname, filelists):
    savefigdir = "/depot/cms/users/jprodger/PUPPI/Physics_Optimization/PhysicsOpt6/perfquant1dropout/"
    # load models
    args = Args()
...
if __name__ == '__main__':
    modelname = "/depot/cms/users/jprodger/PUPPI/Physics_Optimization/PhysicsOpt6/perfquant1dropout/best_valid_model_0.pt"
