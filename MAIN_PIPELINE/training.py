...
def train(dataset, dataset_validation, args):
    ...
    wandb.init(
        project='GraphPUPPI_MAIN_PIPELINE_training',
        config=wandb_config
    )
    ...
    validation_loader = DataLoader(dataset_validation, batch_size=args.batch_size, shuffle=True)
    ...
    while converge == False:
        ...
        for batch in training_loader:
            ...
            for iter in range(rotate_mask):
                ...
                loss = loss / (rotate_mask * accumulation_steps)
            ...
            if step_counter % accumulation_steps == 0:
                ...
                if math.isnan(cur_loss):
                    ...
                    print("label: ", label)
                    print("pred: ", pred)
                cur_loss = cur_loss / (rotate_mask * accumulation_steps)
            ...
            if count_event % batches_per_epoch == 0:
                ...
                if validation_metric >= best_validation_metric:
                    ...
                    if last_steady_event == count_event - batches_per_epoch:
                ...
                if count_event == batches_per_epoch*num_epochs:
                    converge = True
                    break
...
def test(loader, model, indicator, epoch, args, modelcolls, pathname):
    ...
    plt.legend()
    plt.savefig(args.save_dir+"Jet_mass_diff_"+postfix+str(epoch)+".pdf")
    plt.show()
    ...
    plt.legend()
    plt.show()
    plt.savefig(args.save_dir+"Jet_pT_diff_"+postfix+str(epoch)+".pdf")
...
def generate_neu_mask(dataset, args):
    ...
    for graph in dataset:
        ...
        #n_select = min(len(Neutral_index), args.num_select_LV + args.num_select_PU)
        random_mask_index = np.random.choice(Neutral_index, args.num_select_LV + args.num_select_PU, replace = False)
        #random_mask_index = np.random.choice(Neutral_index, n_select, replace = False)
