...
import test_physics_metrics4 as phym
from test_physics_metrics4 import Args
import matplotlib
...
def arg_parse():
    ...
    parser.set_defaults(model_type='Gated',
                        ...
                        save_dir="/depot/cms/users/jprodger/PUPPI/Physics_Optimization/PhysicsOpt17/Wjets/",
                        jet_type = "W"
                        )
...
def train(dataset, dataset_validation, trial, args, batchsize):
    ...
    if args.pulevel == 20:
        ...
        num_select_PU = 282
    '''
    args.dropout = trial.suggest_float("dropout", 0.1, 0.2, step=0.025)
   
    
    num_select_LV = trial.suggest_int("num_select_LV", 1, 30)
    num_select_PU = trial.suggest_int("num_select_PU", 10, 40)
    rotate_mask = trial.suggest_int("rotate_mask", 8, 12)
    
    
    args.hidden_dim = trial.suggest_int("hidden_dim", 30, 45)
    ...
    phym.Args().num_select_LV = num_select_LV
    phym.Args().num_select_PU = num_select_PU
    ...
    while converge == False:
        ...
        for batch in training_loader:
            ...
            if count_event % 1000 == 0:
                ...
                if count_event == 3000 and best_validation_quant >= 0.132:
                    converge = True
                    break
                elif count_event == 6000 and best_validation_quant < 0.132:
                    converge = True
                    break
                elif count_event == 2000 and best_validation_quant >= 0.155:
                    converge = True
                    break
...
def main():
    ...
    fig7.write_image(file = args_end.save_dir + "rank.png", format='png')
