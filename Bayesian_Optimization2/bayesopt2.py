...
def arg_parse():
    ...
    parser.set_defaults(model_type='Gated',
                        ...
                        save_dir="/depot/cms/users/jprodger/PUPPI/Physics_Optimization/PhysicsOpt15/Wjets/",
                        jet_type = "W"
                        )
...
def train(dataset, dataset_validation, trial, args, batchsize):
    ...
    if args.pulevel == 20:
        ...
        num_select_PU = 282
    '''
    args.dropout = trial.suggest_float("dropout", 0.05, 0.25, step=0.025)
    rotate_mask = 9
    
    num_select_LV = trial.suggest_int("num_select_LV", 1, 4)
    num_select_PU = trial.suggest_int("num_select_PU", 20, 30)
    
    
    args.hidden_dim = trial.suggest_int("hidden_dim", 15, 40)
    
    args.lr = trial.suggest_float("lr", 0.001, 0.004, step=0.0002)
    ...
    while converge == False:
        ...
        for batch in training_loader:
            ...
            if count_event % 1000 == 0:
                ...
                if count_event == 2000 and best_validation_quant >= 0.131:
                    converge = True
                    break
                elif count_event == 4000 and best_validation_quant < 0.131:
                    converge = True
                    break
...
def main():
    ...
    study.optimize(black_box_function, n_trials=11)
